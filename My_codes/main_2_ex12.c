  ...
  for (int i =0; i<relCatHeader.numEntries; i++) {
    ...
    while(1){
     ...
     for (int j=0; j<n_attrCatHeader.numEntries; j++) {
       ...
       if(strcmp(relCatRecord[0].sVal,"Events")==0 && flag==0){
        if(strcmp(attrCatRecord[1].sVal,"location")==0){    
         unsigned char buffer[2048];
        Disk:: readBlock(buffer,z);
        int k = 32 + 20 + j*16 + 16;
        memcpy(buffer+k,"city",5);
        strcpy(attrCatRecord[1].sVal,"city");
        Disk :: writeBlock(buffer,z);
        flag = 1;
       }
       }
