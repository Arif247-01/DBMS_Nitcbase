...
//   if(rel_id!=E_RELNOTOPEN) return E_RELOPEN;

//   // Call BlockAccess::deleteRelation() with appropriate argument.
//   int k = BlockAccess::deleteRelation(relName);
...
//     for(int j=i; j<nAttrs; j++){
//       if(strcmp(attrs[i],attrs[j])==0 && i!=j) return E_DUPLICATEATTR;
//     }
//   }
...
//       int retVal2 = BlockAccess::insert(ATTRCAT_RELID, attrCatRecord);
...
//     if(k!=SUCCESS) return E_ATTRNOTEXIST;
