    ...
    while(StaticBuffer::getStaticBlockType(block)==IND_INTERNAL /* block is of type IND_INTERNAL */) {  //use StaticBuffer::getStaticBlockType()
        ...
        if (op==NE || op==LT || op==LE/* op is one of NE, LT, LE */) {
            ...
            */
            ...
            internalBlk.getEntry(&intEntry, 0);   ///doubt
            ...
            */
            ...
            // if (flag == 1/* such an entry is found*/) {
...
int BPlusTree::findLeafToInsert(int rootBlock, Attribute attrVal, int attrType) {
    ...
    while (StaticBuffer::getStaticBlockType(blockNum)!=IND_LEAF/*block is not of type IND_LEAF */) {  // use StaticBuffer::getStaticBlockType()
        ...
        for(int i=0; i<head.numEntries; i++){
            obj.getEntry(&entry, i);
            int k = compareAttrs(entry.attrVal, attrVal, attrType);
            if(k>=0){
                flag = 1;
                index = i;
                break;
...
int BPlusTree::insertIntoLeaf(int relId, char attrName[ATTR_SIZE], int blockNum, Index indexEntry) {
    ...
    */
    ...
    for (int i = 0; i < flag; i++){
        indLeaf.getEntry(&indices[i], i);
    }

    indices[flag] = indexEntry;

    for (int i = flag; i < numEntries; i++){
        indLeaf.getEntry(&indices[i + 1], i);
    }
...
int BPlusTree::insertIntoInternal(int relId, char attrName[ATTR_SIZE], int intBlockNum, InternalEntry intEntry) {
   ...
   int flag = numEntries;
    ...
    if (flag < numEntries)
    {
        internalEntries[flag + 1].lChild = internalEntries[flag].rChild;
    }
...
int BPlusTree::splitInternal(int intBlockNum, InternalEntry internalEntries[]) {
   ...
   for(int i=51; i<101; i++){
    ...
    int type = StaticBuffer::getStaticBlockType(internalEntries[50].lChild)/* block type of a child of any entry of the internalEntries array */;
    //            (use StaticBuffer::getStaticBlockType())
    InternalEntry entry;
    leftBlk.getEntry(&entry, 0);
    BlockBuffer buffer(entry.lChild);
    HeadInfo head;
    ...
    for (int i=0; i<rightBlkHeader.numEntries; i++/* each child block of the new right block */) {
        ...
        BlockBuffer blockBuffer(entry.rChild);

        // update pblock of the block to rightBlkNum using BlockBuffer::getHeader()
        // and BlockBuffer::setHeader().
        HeadInfo headz;
        blockBuffer.getHeader(&headz);
        headz.pblock = rightBlkNum;
        blockBuffer.setHeader(&headz);
    }
...
int BPlusTree::createNewRoot(int relId, char attrName[ATTR_SIZE], Attribute attrVal, int lChild, int rChild) {
    ...
    IndInternal newRootBlk;  ///doubt..

    int newRootBlkNum = newRootBlk.getBlockNum()/* block number of newRootBlk */;  //doubt...
    ...
    BlockBuffer l(lChild);
    l.getHeader(&head);
    head.pblock = newRootBlkNum;
    l.setHeader(&head);

    BlockBuffer r(rChild);
    r.getHeader(&head);
    head.pblock = newRootBlkNum;
    r.setHeader(&head);
    ...
    while (block != -1) {
        ...
        for(int i = 0; i < relCatEntry.numSlotsPerBlk; i++){
            if(slotMap[i]==SLOT_OCCUPIED)
            {
                ...
                int retVal = bPlusInsert(relId, attrName, record[attrCatEntry.offset], recId); ///doubt...
...
int BPlusTree::bPlusDestroy(int rootBlockNum) {
    ...
    if (type == IND_LEAF) {
        ...
        for(int i=0; i<head.numEntries; i++){
            ...
            indInternal.getEntry(&entry1, i);
            BPlusTree::bPlusDestroy(entry1.rChild);
        }
...
int BPlusTree::bPlusInsert(int relId, char attrName[ATTR_SIZE], Attribute attrVal, RecId recId) {
    ...
    Index index;
    index.attrVal = attrVal;   //doubt...
    index.block = recId.block;
    index.slot = recId.slot;
    int kx = insertIntoLeaf(relId, attrName, leafBlkNum, index);
    ...
    if (kx == E_DISKFULL/*insertIntoLeaf() returns E_DISKFULL */) {
